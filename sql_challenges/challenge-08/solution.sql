...
-- c) Would adding an index on site_id help? Why or why not?
    -- Yes, because it would give it more cardinality
...
-- d) Why does the range size affect whether Oracle uses the index?
    -- It doesn't affect it here due to the fact that there are many repeating dates
...
--    Does the composite index get used? Why not?
    -- No, because the composite index is used only when searching for both visit_date and patient_id
-- c) What's the rule about column order in composite indexes?
    -- That it does matter because indexes are most often more effective from the left side
...
-- b) Why does wrapping a column in a function break index use?
    -- Because the function is tranforming the data in the column, and this tranformed data won't appear in the index
-- c) How would you rewrite the second query to allow index use?
    -- Make an index explicitly with the function already, instead of the base column
...
-- → Index on date? Yes/No, why?
    -- Yes, because the table can have dates that repeat
...
-- → What indexes would you add?
    -- A composite index with customer_id, order_status
